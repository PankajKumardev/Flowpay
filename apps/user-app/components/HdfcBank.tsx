    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <header className="bg-[#004C8F] text-white p-4">
        <div className="container mx-auto flex justify-between items-center">
          <Image src={hdfc} alt="HDFC Bank Logo" width={120} height={40} className="" />
          <nav>
      ...
      <main className="flex-grow container mx-auto mt-8 p-4 flex items-center justify-center">
        <div className="bg-white shadow-md rounded-lg p-8 max-w-md w-full">
          <h1 className="text-2xl font-bold text-[#004C8F] mb-6 text-center">Transaction Processing</h1>
          {isLoading ? (
            <div className="flex flex-col items-center justify-center space-y-4 py-8">
              <Loader2 className="h-16 w-16 animate-spin text-[#004C8F]" />
              <p className="text-lg font-semibold text-[#004C8F]">Processing your transaction...</p>
              <p className="text-sm text-gray-600">Please do not refresh or close this page.</p>
            </div>
          ) : isComplete ? (
            ...
            <div className="text-center space-y-4 py-8">
              ...
              <p className="text-lg">{message}</p>
              <p className="text-sm text-gray-600">Please try again or contact customer support.</p>
            </div>
          )}
        </div>
      </main>
      <footer className="bg-[#004C8F] text-white text-sm p-4 mt-8">
        <div className="container mx-auto text-center">
          © 2023 HDFC Bank Ltd. All rights reserved. | Terms and Conditions | Privacy Policy
        </div>
      </footer>
    </div>
  )
